...
class PredatorBullet(pygame.sprite.Sprite):
    def __init__(self, x, y, images, speed=5):
        super().__init__()
        self.images = images  # List of bullet images (3 images of different sizes)
        self.image = self.images[1]  # Start with the smallest image
        self.rect = self.image.get_rect(center=(x, y))
        self.speed = speed
        self.x, self.y = x, y
    ...
    def update(self):
        ...
        # Change image based on the bullet's position on the screen
        if self.y < SCREEN_HEIGHT // 3:
            self.image = self.images[0]  # Smallest image
